// The "health" add-on's data API — Apple Watch health time-series, folded onto the CORE
...
// assertAddonEnabled(db,"health") INSIDE mutate() — a disabled add-on throws NotFoundError
...
// aggregators below MUST stay in lockstep with what board/app/api/health/push writes.
 ...
 * GATED: throws NotFoundError (→ 404) when the "health" add-on is disabled. The caller
 ...
 * caller (the athlete route) supplies already-validated/coerced fields; the store helper
