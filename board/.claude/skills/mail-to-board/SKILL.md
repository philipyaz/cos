...
| **Cancellation notice for a scheduled meeting** (an organiser's "Événement annulé…" / "Cancelled: …" notice, or the counterparty calling it off in the thread) | Find the **same** event on the board calendar (the linked case's `events` array, or `list_events` around the date and match title/time), then **`update_event` with `status: "cancelled"`** (via the **`calendar`** MCP). The event **stays** on the calendar as the record — it renders struck-through and stops blocking placement. **Never `delete_event`** for a cancellation, and never a prose note asking a human to remove it. A pure **reschedule** (new time, same meeting) is `update_event` with the new `date`/`startTime`/`endTime` on the same event — never a second event. |
