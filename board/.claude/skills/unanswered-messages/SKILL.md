...
## Board API operations — the four tools' contract

Every board write goes **only** through the `board` MCP (Steps 4–5); here is the exact contract so a
call lands first try and you know how to react when it doesn't.
...
- **The API does NOT dedup.** `POST /api/messages` always mints a fresh `M-<n>` and appends it — there
  is **no** server-side de-duplication. "One reply-owed = one record" is **your** job (Step 4): always
