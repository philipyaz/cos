...
| **[`/mail-to-board`](./mail-to-board/SKILL.md)** | Reconciles your Gmail (received **and** sent) onto the board — links messages to cases, advances tasks, moves lanes, dedups — and **never** undoes your manual edits. Drops noise with a per-sender decision receipt. | `Run /mail-to-board` | ~10–15 min |
| **[`/whatsapp-triage`](./whatsapp-triage/SKILL.md)** | The same reconciliation for **WhatsApp** (DMs **and** groups), turning chats into tracked cases on the board — and capturing explicit "we're out of X" purchase statements onto the **shopping list** (fail-closed, with per-message provenance). | `Run /whatsapp-triage` | ~15 min |
...
| **[`/reminders-review`](./reminders-review/SKILL.md)** | Reviews every **open reminder** and **closes** the ones already done or past their moment — auto-closing only what's *proven* done (finished checklist, linked case closed, delivery date passed) and **proposing** the rest (cold job-alerts, lapsed RSVPs) — so the reminders list stays a live to-do surface, not a graveyard. Also surfaces newly filtered senders (the mail sweep's drops) for a keep-or-confirm. | `Run /reminders-review` | daily / every few hours |
| **[`/nutrition-chef`](./nutrition-chef/SKILL.md)** | Takes stock of the pantry — what's expired, what's likely past its freshness horizon, what this week's plan needs — and drafts the weekend **shopping list**, after the standing meal-plan reconcile. | `Run /nutrition-chef take stock of the pantry and draft the shopping list` | Friday (before the weekend shop) |
...
| **[`/fitness-training-plan`](./fitness-training-plan/SKILL.md)** | Authors + persists the **new week's training plan** — grounded in the athlete profile, the body goal, the recovery state, the last ~4 weeks of workouts and the vault's recorded context, varied against the last few plans with progressive overload toward the goal date — upserted by ISO week. | `Run /fitness-training-plan` | weekly (Sunday evening / Monday morning) |
| **[`/fitness-weekly-review`](./fitness-weekly-review/SKILL.md)** | The look-back over the training week — planned vs done, sleep / HRV / resting-HR trends, the daily form scores, the vault's recorded context — authored into a scored review with 3–5 recommendations and a next-week focus, persisted per ISO week. | `Run /fitness-weekly-review` | weekly (end of week, before the new plan) |
