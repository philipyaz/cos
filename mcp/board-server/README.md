...
(see [`mcp/CLAUDE.md`](../CLAUDE.md) and the `/mcp-bridge-setup` skill). Option A below shows what
those generators produce; Option B is a hand-written stdio alternative for local testing.

### Option A — HTTP via supergateway (the bridged setup)

Front this server with supergateway on the host, on port **8001**. The LaunchAgent plist is
generated from `mcp/board-server/board.service.json` by `scripts/gen-launchd.mjs` (see
[`mcp/CLAUDE.md`](../CLAUDE.md)); see the `/mcp-bridge-setup` skill for the runbook. The
