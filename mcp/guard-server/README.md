...
`/mcp-bridge-setup` / `/guard-setup` skills. Option A below shows what those generators produce;
Option B is a hand-written stdio alternative for local testing.

### Option A — HTTP via supergateway (the bridged setup)

Front this server with supergateway on the host, on port **8004**. The LaunchAgent plist is
generated from `mcp/guard-server/guard.service.json` by `scripts/gen-launchd.mjs` (see
[`mcp/CLAUDE.md`](../CLAUDE.md)); see the `/mcp-bridge-setup` / `/guard-setup` skills for the
runbook. The `.mcp.json` entry below is generated by `scripts/gen-mcp-json.mjs` (committed +
CI-checked):
