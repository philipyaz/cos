...
skill). Option A below shows what those generators produce; Option B is a hand-written stdio
alternative for local testing.

### Option A — HTTP via supergateway (the bridged setup)

Front this server with supergateway on the host, on port **8003**. The LaunchAgent plist is
generated from `mcp/calendar-server/calendar.service.json` by `scripts/gen-launchd.mjs` (see
