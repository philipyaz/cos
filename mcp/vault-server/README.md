...
The plist is generated from `mcp/vault-server/vault.service.json` by `scripts/gen-launchd.mjs`
(see [`mcp/CLAUDE.md`](../CLAUDE.md)): its `ProgramArguments` runs the **`launch.sh`** secret
wrapper first — it sources `ANTHROPIC_API_KEY` from the gitignored `config/secrets.env` on
every start, then execs the shared supergateway argv rendered after it (loopback preload
included, so the bridge listens on 127.0.0.1 only). The key never lands in the installed
plist. See the `/mcp-bridge-setup` skill for the full install; install/refresh with
`node scripts/gen-launchd.mjs --install vault`. The `.mcp.json` entry below is generated by
`scripts/gen-mcp-json.mjs` (committed + CI-checked):
