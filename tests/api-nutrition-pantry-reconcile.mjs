...
//   COS_BOARD_DATA=<that board's cases.json> node tests/api-nutrition-pantry-reconcile.mjs    # CRM_BASE_URL defaults to http://localhost:3000
//
...
async function main() {
  ...
  try {
    ...
    // 9. Restore the snapshot regardless of outcome — only when there is one to restore.
