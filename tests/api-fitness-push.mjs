#!/usr/bin/env node
// api-fitness-push.mjs — a round-trip through the fitness-push INGEST → SUMMARIZE pipeline that
...
//   1. POST /api/fitness/push a realistic Health-Auto-Export-shaped payload:
//        • a metrics export with a `sleep_analysis` night + a `heart_rate_variability` series
//        • a workouts export with one run
//   2. GET /api/fitness/summary?date=… → assert NON-EMPTY sleep {count,avg_hours} + hrv
...
// settings.addons live there) — but ONLY when COS_BOARD_DATA is set: unset means no
...
//   COS_BOARD_DATA=<that board's cases.json> node tests/api-fitness-push.mjs   # CRM_BASE_URL defaults to :3000
//
// Env: CRM_BASE_URL, COS_BOARD_DATA (the RUNNING board's cases.json — snapshot/restore SKIPs if
// unset).
...
async function main() {
  ...
  try {
    ...
    const summary = await GET(`/api/fitness/summary?date=${DAY}`);
    check(summary.status === 200, `GET /api/fitness/summary → 200 (got ${summary.status})`);
    ...
    const daily = await GET(`/api/fitness/daily-summary?date=${DAY}`);
    check(daily.status === 200, `GET /api/fitness/daily-summary → 200 (got ${daily.status})`);
    check(
  ...
  console.log("\nPASS — fitness push→summarize round-trip surfaces sleep + hrv + workout (canonical taxonomy).");
}
