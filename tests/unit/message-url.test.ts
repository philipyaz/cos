...
test("messageDeepLink — prefers the structured url field when set", () => {
  const url = "https://mail.google.com/mail/u/0/#all/f00d01";
...
test("messageDeepLink — falls back to a Gmail thread URL embedded in the preview", () => {
  ...
  const preview = "A contact shared a doc — https://mail.google.com/mail/u/0/#all/f00d01";
  assert.equal(messageDeepLink({ preview }), "https://mail.google.com/mail/u/0/#all/f00d01");
});

test("messageDeepLink — falls back to a Gmail thread URL embedded in the body ('Link: <url>')", () => {
  const body = "…agreed). Link: https://mail.google.com/mail/u/0/#all/f00d02";
  assert.equal(messageDeepLink({ body }), "https://mail.google.com/mail/u/0/#all/f00d02");
});
...
test("messageDeepLink — supports #inbox/<id> and #label/X/<id> Gmail views, not just #all", () => {
  assert.equal(
    messageDeepLink({ preview: "x — https://mail.google.com/mail/u/2/#inbox/f00d03" }),
    "https://mail.google.com/mail/u/2/#inbox/f00d03",
  );
  assert.equal(
    messageDeepLink({ body: "Link: https://mail.google.com/mail/u/0/#label/Work/f00d04" }),
    "https://mail.google.com/mail/u/0/#label/Work/f00d04",
  );
});
