#!/usr/bin/env node
// api-fitness-coaching.mjs — full CRUD + gate + upsert + token contract for the "fitness"
// add-on's STATEFUL coaching artifacts (/api/fitness/coaching[/:id] + db.coachingArtifacts).
//
...
// (kind, periodKey), creatable by an EXTERNAL agent (Cowork) via a TOKEN-GATED POST WITHOUT
// the board's Anthropic key, and queryable as history via GET. Against a RUNNING board with
// the "fitness" add-on ENABLED this proves:
//   • POST /api/fitness/coaching (x-fitness-token + a valid training_plan body) → 201; the
//     minted artifact.id starts "COACH-"; the response carries { artifact, version, created:true }
...
//   • POST WITHOUT x-fitness-token → 401 (the token gate sits at the EDGE, before validate/mutate)
//   • DISABLE the add-on (PATCH /api/addons/fitness { enabled:false }) → a tokened POST 404s
//     (writes close) while GET /api/fitness/coaching stays 200 (reads stay open)
//   • re-ENABLE → DELETE /api/fitness/coaching/<id> with the token → ok; GET <id> → 404
//
// The coaching write routes are token-gated (x-fitness-token must match FITNESS_PUSH_TOKEN).
// The test board exports FITNESS_PUSH_TOKEN (see tests/run.sh); locally pass it via env. When
// unset the write routes 503 — the test SKIPs gracefully (exit 0), it cannot exercise the CRUD.
//
...
//   FITNESS_PUSH_TOKEN=… node tests/api-fitness-coaching.mjs   # CRM_BASE_URL defaults to :3000
//
...
// The token header the coaching write routes require (x-fitness-token), and a valid
// training_plan body (the minimal required fields validateCoachingArtifactInput enforces:
// a non-empty week string + a days array). The week is the upsert periodKey.
const TOKEN_HEADER = () => ({ "x-fitness-token": FITNESS_TOKEN });
const WEEK = "2026-W26";
...
async function main() {
  ...
  if (!FITNESS_TOKEN) {
    console.log("  SKIP: FITNESS_PUSH_TOKEN unset — the coaching write routes are token-gated, cannot CRUD.");
    process.exit(0);
  }

  const snapshot = await fs.readFile(DATA_FILE, "utf8");
  const H = TOKEN_HEADER();
  ...
  try {
    ...
    // ── CREATE: a tokened POST mints a COACH-<n> artifact ────────────────────
    const created = await POST("/api/fitness/coaching", trainingPlanBody(), H);
    ...
    const upsert = await POST("/api/fitness/coaching", trainingPlanBody(), H);
    ...
    // ── TOKEN GATE: a POST WITHOUT the token is 401 (edge gate, pre-validate) ─
    const noToken = await POST("/api/fitness/coaching", trainingPlanBody());
    check(noToken.status === 401, `POST without x-fitness-token → 401 (got ${noToken.status})`);
    ...
    const blockedPost = await POST("/api/fitness/coaching", trainingPlanBody(), H);
    check(blockedPost.status === 404, `POST while disabled (with token) → 404 (got ${blockedPost.status})`);
    ...
    const del = await DELETE(`/api/fitness/coaching/${encodeURIComponent(id)}`, H);
    check(del.status === 200, `DELETE /api/fitness/coaching/${id} (with token) → 200 (got ${del.status})`);
  ...
  console.log("\nPASS — fitness coaching artifacts CRUD holds (201 create, upsert-no-duplicate, 401 no-token, 404 disabled, reads open, delete).");
}
