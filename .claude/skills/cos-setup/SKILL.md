...
- **What it does** — writes `config/cos.env`, the machine-local **public** config every skill
  ...
  **only** these public values — **no secrets**. The config split is deliberate, four files under
  ...
  - **`secrets.env`** — the `ANTHROPIC_API_KEY` (step 3), plus the optional `FITNESS_PUSH_TOKEN` for
    the fitness add-on. The loader does **not** source it; each bridge's own launch wrapper does
    (vault's `launch.sh` for the API key, fitness's `launch.sh` for the push token).
