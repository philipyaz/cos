---
name: health-mcp-setup
description: Stand up the `health` MCP for the "Health & Athlete" add-on on a new machine and wire it into both Claude clients — an in-repo thin fetch-wrapper over the board's /api/health/* routes (like the nutrition/calendar servers), with ONE machine-local secret (HEALTH_PUSH_TOKEN) it attaches as the `x-health-token` header on its write tools. It is exposed to Claude Code via a supergateway + launchd BRIDGE on $HEALTH_BRIDGE_PORT/:8011 and to Claude Cowork Desktop as a direct stdio command, and the add-on must be ENABLED (Settings.addons.health.enabled) for its writes to land + its /health + /athlete nav to appear. Use when setting up the health/athlete add-on on a new machine, when Cowork or Code can't see the `health` server, when the health bridge (:8011) is down, when push_health_data / delete_health_data write but the board 404s or 401s them, or when enabling/disabling the Health & Athlete add-on.
---

# Health & Athlete MCP setup (thin fetch-wrapper bridge :8011, one ingest secret)

## Why this exists / architecture
`health` is the **second built-in add-on** ("Health & Athlete" — Apple Watch health ingestion +
dashboard at `/health`, plus the athlete training profile + AI coach at `/athlete`). Like
**nutrition**, it has **no external repo and no sidecar**: the server lives **in this Cos checkout**
at `mcp/health-server/server.mjs`, and it is the same **thin `fetch` wrapper** archetype over the
board's `/api/health/*` HTTP routes on `CRM_BASE_URL`. It makes **no LLM calls** — the *coaching*
intelligence (training plans, weekly reviews, pre-workout briefs) lives on the board's `/athlete/*`
routes (server-side Claude calls) and in the operator skill (`/athlete-coach`), not here.

The **one difference from nutrition**: this server carries a single machine-local **secret**,
`HEALTH_PUSH_TOKEN`. The board's ingest endpoint (`POST /api/health/push`) is **token-gated** via
the `x-health-token` header — the same token the iPhone Health Auto Export shortcut sends — and the
...
- **Claude Code** reaches it over **HTTP** via a **supergateway + launchd BRIDGE on `:8011`**
  (`$HEALTH_BRIDGE_PORT`, label `com.chiefofstaff.mcp-health`, IN `.mcp.json`).
...
```
Claude Code   ──HTTP──> localhost:8011/mcp ──supergateway(launchd)──> node health-server (stdio)
                                                                          │ fetch (+ x-health-token on writes)
                                                                          └──HTTP──> board /api/health/* (:3000)
Cowork Desktop ──spawns stdio directly──────────────────────────────────> node health-server   (§5)
```

> **This is an ADD-ON — its WRITES are GATED, its READS are open.** The "Health & Athlete" add-on
> must be **ENABLED** (`Settings.addons.health.enabled` in `cases.json`) for **writes** to land. A
> disabled add-on **404s every write** (the board surfaces it here as a `Not found — the health
> add-on may be disabled.` tool error) while every **GET read stays open**, and its `/health` +
> `/athlete*` nav pages 404 + hide. Enabling is itself a step (§6) — flip it from the board's
> **`/addons`** catalog (or `PATCH /api/addons/health`).
...
|---|---|---|---|---|---|
| health (MCP bridge) | `node mcp/health-server/server.mjs` via supergateway | `CRM_BASE_URL` + `HEALTH_PUSH_TOKEN` | 8011 | `com.chiefofstaff.mcp-health` | **yes** (`http://localhost:8011/mcp`) |
...
(`push_health_data` / `delete_health_data`) attach `x-health-token` and are gated; the 5 reads are
ungated. See `mcp/health-server/README.md` for the full contract.
...
> `$BOARD_URL`, and the **health keys** — `$HEALTH_BRIDGE_PORT` (=`8011`) /
> `$HEALTH_BRIDGE_URL` (=`http://localhost:8011`) — use those instead of hardcoding paths, the
> Homebrew prefix, your username, or the port. The **secret** (`HEALTH_PUSH_TOKEN`) is read from
> `config/secrets.env` (§4), not `cos.env`.

## Prerequisites
- **Node + npm** (Homebrew) and the server's deps: `(cd mcp/health-server && npm i)`. The server
...
- **The board** reachable at `CRM_BASE_URL` (default `http://localhost:3000`). The server is a pure
  wrapper over the board's `/api/health/*` routes — there is no other backend, no external app, no
  pairing, no login. (Unlike whatsapp/openwhispr, there is **nothing external** to install.)
- A **`HEALTH_PUSH_TOKEN`** in `config/secrets.env` (§4). Any random string — it's machine-local and
  never committed.
...
```sh
source "$(git rev-parse --show-toplevel)/config/load-config.sh"
echo "HEALTH_BRIDGE_PORT=$HEALTH_BRIDGE_PORT  ($HEALTH_BRIDGE_URL)"   # expect 8011
(cd "$REPO_ROOT/mcp/health-server" && npm i >/dev/null 2>&1)
```
> `HEALTH_BRIDGE_PORT` is already in `config/cos.env.example` + `config/load-config.sh` (default
...
- **CHECKPOINT** — the port resolves and the server's deps are present:
  ```sh
  source "$(git rev-parse --show-toplevel)/config/load-config.sh"
  test "${HEALTH_BRIDGE_PORT:-}" = 8011 && echo "port OK (or note your override)"
  ls "$REPO_ROOT/mcp/health-server/server.mjs" && echo "server present"
  ```

### 2. Verify the stdio server runs
```sh
source "$(git rev-parse --show-toplevel)/config/load-config.sh"
CRM_BASE_URL="$BOARD_URL" "$NODE_BIN" "$REPO_ROOT/mcp/health-server/server.mjs"   # Ctrl-C
# prints "health MCP server v1 ready (tools: push_health_data, list_health_data, …; CRM_BASE_URL=…)"
```
...
### 3. Ensure the `.mcp.json` health http entry (Claude Code)
The repo already ships the `health` http entry in `$REPO_ROOT/.mcp.json`. Confirm it points at the
`:8011` bridge — do **not** clobber the other servers:
```json
{ "mcpServers": {
  "health": { "type": "http", "url": "http://localhost:8011/mcp" }
}}
```
- **CHECKPOINT** — the entry is present alongside the core servers:
  ```sh
  source "$(git rev-parse --show-toplevel)/config/load-config.sh"
  grep -q '"health"' "$REPO_ROOT/.mcp.json" && echo ".mcp.json health entry OK"
  ```
  If it is somehow missing, add the one entry above (merged into the existing `mcpServers`).

### 4. Seed the ingest secret (`HEALTH_PUSH_TOKEN`)
...
sends as `x-health-token`, so reuse it there too:
```sh
source "$(git rev-parse --show-toplevel)/config/load-config.sh"
SEC="$REPO_ROOT/config/secrets.env"
[ -f "$SEC" ] || cp "$REPO_ROOT/config/secrets.env.example" "$SEC"
grep -q '^HEALTH_PUSH_TOKEN=' "$SEC" || printf 'HEALTH_PUSH_TOKEN=%s\n' "$(openssl rand -hex 20)" >> "$SEC"
# If it's still the placeholder, replace it with a real random token:
grep -q '^HEALTH_PUSH_TOKEN=replace-with-a-random-token' "$SEC" \
  && { TOK=$(openssl rand -hex 20); sed -i '' "s/^HEALTH_PUSH_TOKEN=.*/HEALTH_PUSH_TOKEN=$TOK/" "$SEC"; } || true
```
> The board side reads the SAME token: the ingest route validates `x-health-token` against
> `HEALTH_PUSH_TOKEN` from the environment the **board** process sees (it sources `config/secrets.env`
...
- **CHECKPOINT** — a real (non-placeholder) token is present:
  ```sh
  source "$(git rev-parse --show-toplevel)/config/load-config.sh"
  grep -q '^HEALTH_PUSH_TOKEN=' "$REPO_ROOT/config/secrets.env" \
    && ! grep -q '^HEALTH_PUSH_TOKEN=replace-with-a-random-token' "$REPO_ROOT/config/secrets.env" \
    && echo "HEALTH_PUSH_TOKEN seeded"
  ```

### 5. Install the launchd BRIDGE (`:8011`)
Install from the **committed template**
`mcp/health-server/deploy/com.chiefofstaff.mcp-health.plist.template` — same pattern as the
...
`secrets.env` first so `$HEALTH_PUSH_TOKEN` is in scope:
```sh
source "$(git rev-parse --show-toplevel)/config/load-config.sh"; U=$(id -u)
set -a; . "$REPO_ROOT/config/secrets.env"; set +a   # export HEALTH_PUSH_TOKEN for the sed below
...
sed -e "s#__BREW_PREFIX__#$BREW_PREFIX#g" \
  ...
  "$REPO_ROOT/mcp/health-server/deploy/com.chiefofstaff.mcp-health.plist.template" \
  > "$LAUNCH_AGENTS_DIR/com.chiefofstaff.mcp-health.plist"
launchctl bootout   gui/$U/com.chiefofstaff.mcp-health 2>/dev/null || true
launchctl bootstrap gui/$U "$LAUNCH_AGENTS_DIR/com.chiefofstaff.mcp-health.plist"
launchctl kickstart -k gui/$U/com.chiefofstaff.mcp-health
```
...
- `HEALTH_PUSH_TOKEN=…` — the substituted-in ingest secret (the one machine-local credential),
...
- **CHECKPOINT** — an MCP `initialize` on `:8011` returns `serverInfo.name == "health"`:
  ```sh
  source "$(git rev-parse --show-toplevel)/config/load-config.sh"
  curl -s -X POST "$HEALTH_BRIDGE_URL/mcp" \
    ...
    | grep -o '"name":"health"' && echo "health bridge OK"
  ```
...
and refreshes only the `health` entry from the **resolved** loader values + the secret — mirroring
...
set -a; . "$REPO_ROOT/config/secrets.env"; set +a                  # HEALTH_PUSH_TOKEN into env for the merge
"$NODE_BIN" - <<'NODE'
const fs = require('node:fs'), E = process.env;
const server = {
  command: E.NODE_BIN,
  args: [`${E.REPO_ROOT}/mcp/health-server/server.mjs`],
  env: {
    CRM_BASE_URL: E.BOARD_URL || 'http://localhost:3000',  // the board
    HEALTH_PUSH_TOKEN: E.HEALTH_PUSH_TOKEN || '',           // the ingest secret; NO idle-exit here
  },
};
const p = E.COWORK_CONFIG, cfg = fs.existsSync(p) ? JSON.parse(fs.readFileSync(p,'utf8')) : {};
cfg.mcpServers = { ...(cfg.mcpServers||{}), health: server };     // refresh OURS; keep other servers + preferences intact
...
- **Quit + reopen Claude Desktop (⌘Q)** — it reads this file only at launch; then `health`
  appears in Cowork's tools.
- **CHECKPOINT** — both registrations present:
  ```sh
  source "$(git rev-parse --show-toplevel)/config/load-config.sh"
  grep -q '"health"' "$REPO_ROOT/.mcp.json" && echo ".mcp.json OK"
  "$NODE_BIN" -e 'const c=require(process.env.COWORK_CONFIG);process.exit(c.mcpServers&&c.mcpServers.health?0:1)' \
...
`/health` + `/athlete*` nav + pages are hidden. Flip it on — either the **`/addons` UI toggle** (with
the board app running) or the API (the toggle the UI calls):
```sh
source "$(git rev-parse --show-toplevel)/config/load-config.sh"
curl -s -X PATCH "$BOARD_URL/api/addons/health" \
  -H 'Content-Type: application/json' -d '{"enabled":true}' ; echo
# → {"addon":{"id":"health","enabled":true}, "version":<n>}
```
Enabling flips `Settings.addons.health.enabled` to `true` in `cases.json`, bumps `db.version` → SSE,
so the sidebar's **Add-ons** nav group + the `/health` + six `/athlete*` pages light up live (no
reload), and writes start landing.
> **Soft dependency on Nutrition.** The health manifest declares `dependsOn: [{id:"nutrition",
...
> reads empty), so you do **not** need the Nutrition add-on enabled for Health to work — it just
> works *better* with it (the catalog surfaces this as "works better with Nutrition").
- **CHECKPOINT** — the catalog reports it enabled:
  ```sh
  source "$(git rev-parse --show-toplevel)/config/load-config.sh"
  curl -s "$BOARD_URL/api/addons" | grep -o '"id":"health"[^}]*"enabled":true' && echo "add-on enabled"
  ```
...
```
If you call (a) BEFORE §7 you'll see `Not found — the health add-on may be disabled.` — that's the
gate working, not a failure; enable the add-on and retry. A `401 / Unauthorized — check
HEALTH_PUSH_TOKEN` means the bridge's token doesn't match the board's (rotate both + restart the
board, §4).
- **CHECKPOINT** — `push_health_data` returns an `accepted` count, `list_health_data` echoes the
  entry, `delete_health_data` returns `deleted`, and both clients see `health` after a ⌘Q reopen. The
  ...
  calendar, ingesting to the vault) is then owned by **`/athlete-coach`** — that skill owns the
  operation; this one only proves the plumbing.

### 9. (Optional) confirm `mcp/ensure-bridges.sh` brings it up
`health` is **already wired** into `mcp/ensure-bridges.sh` as an **OPTIONAL** bridge (`"health
${HEALTH_BRIDGE_PORT:-8011}"` in the service list; in the
`openwhispr|whatsapp|whatsappbridge|vaultjobs|nutrition|health` skip-if-no-plist case), so `npm run
...
```sh
source "$(git rev-parse --show-toplevel)/config/load-config.sh"
"$REPO_ROOT/mcp/ensure-bridges.sh" | grep -i health   # expect: "[mcp] health bridge up on :8011"
```

## Manage
```sh
launchctl kickstart -k gui/$(id -u)/com.chiefofstaff.mcp-health   # restart the bridge
launchctl bootout      gui/$(id -u)/com.chiefofstaff.mcp-health   # stop the bridge
tail -f mcp/logs/health.err.log                                   # supergateway / server log
```
**Rotate the token:** edit `HEALTH_PUSH_TOKEN` in `config/secrets.env`, **restart the board** (so it
...
```sh
source "$(git rev-parse --show-toplevel)/config/load-config.sh"
curl -s -X PATCH "$BOARD_URL/api/addons/health" -H 'Content-Type: application/json' -d '{"enabled":false}' ; echo
```

## --uninstall (remove the add-on plumbing — the inverse of §3–§7)
Tear the bridge + registrations down (a machine that won't run health). This is the inverse of the
...
curl -s -X PATCH "$BOARD_URL/api/addons/health" -H 'Content-Type: application/json' -d '{"enabled":false}' >/dev/null 2>&1 || true
# 2. Stop + remove the launchd bridge (the rendered plist carried the token — remove it):
launchctl bootout gui/$U/com.chiefofstaff.mcp-health 2>/dev/null || true
rm -f "$LAUNCH_AGENTS_DIR/com.chiefofstaff.mcp-health.plist"
...
if (fs.existsSync(p)) {
  ...
  if (cfg.mcpServers) delete cfg.mcpServers.health;
...
NODE
echo "Uninstalled the health bridge + both registrations. ⌘Q + reopen Cowork to drop it from the tool list."
```
> The `HEALTH_PUSH_TOKEN` line stays in `config/secrets.env` (harmless; re-enabling reuses it). The
> repo SHIPS the `.mcp.json` health entry and the `ensure-bridges.sh` line; removing them from a
> tracked file leaves a local diff. For a per-machine opt-out, leaving the plist uninstalled is
> enough — `ensure-bridges.sh` skips health silently when no plist exists, so a stray `.mcp.json`
...
- **The add-on must be ENABLED for writes; reads are always open.** The gate is
  `Settings.addons.health.enabled` in `cases.json` (`assertAddonEnabled` inside the board's
  `mutate()` lock). A disabled add-on 404s every WRITE (surfaced as `Not found — the health add-on
  may be disabled.`) and hides/404s the `/health` + `/athlete*` pages, but GET reads stay open and
...
- **The token is the SECOND auth layer — distinct from the add-on gate.** A `401` (not `404`) means
  the bridge's `HEALTH_PUSH_TOKEN` doesn't match what the board sees. Both must carry the SAME value:
...
- **The COACHING intelligence is NOT in the MCP.** The 7 tools just push/read health data + compose a
  ...
  `/athlete/*` routes (server-side Claude calls) and orchestrated by the operator skill
  (`/athlete-coach`). Don't expect an MCP tool to "generate a plan".
...
- **No sidecar, no external repo, ONE secret.** Unlike whatsapp (Go + Python, external checkout, QR
  pairing) and openwhispr (external app store), health is the **nutrition archetype plus a token**:
  an in-repo Node `fetch` wrapper that needs the board on `CRM_BASE_URL` and `HEALTH_PUSH_TOKEN` for
  its writes. There is nothing to clone, build, or pair.
- **The board must be reachable on `CRM_BASE_URL`.** Every tool is a `fetch` to `/api/health/*`. If
...
- **Port `8011` must be free** (`lsof -nP -iTCP:8011 -sTCP:LISTEN`). It sits after the search/guardsvc
  ...
  — change `HEALTH_BRIDGE_PORT` in `cos.env` and re-render §3/§5.
