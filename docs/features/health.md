# Health & Athlete — the training add-on

Cos's core is about *matters* — work and life to-dos on a board. **Health & Athlete** is the
...
- **`/health` — the dashboard.** Apple Watch HealthKit data — workouts, sleep, HRV, resting heart
  rate, steps, VO₂max — ingested off your phone and read back as a daily summary, trends, and a
  Markdown report.
- **`/athlete` — the coach.** A singleton training profile (your goal, level, available days,
...
**writes in** from the watch (via a token-gated push) and the **agent reads** through the health
...
Like [nutrition](nutrition.md) and the [calendar](calendar.md), health is **not a new store**. It
...
enums) and
[`board/lib/health.ts`](https://github.com/philipyaz/cos/blob/main/board/lib/health.ts) (the data
API — the module that retired `health-store.ts`).

## HAE ingestion — the `x-health-token` push model
...
on a schedule and `POST`s it to **`/api/health/push`**
([`board/app/api/health/push/route.ts`](https://github.com/philipyaz/cos/blob/main/board/app/api/health/push/route.ts)).
...
- The request must carry **`x-health-token`** matching **`HEALTH_PUSH_TOKEN`** from the server env
  (lives in `config/secrets.env`).
- A missing token on the server → **`503`** ("not configured"); a wrong/absent header → **`401`**.
- The add-on gate still applies: `pushEntries` calls `assertAddonEnabled(db, "health")` **inside
...
[health MCP server](https://github.com/philipyaz/cos/blob/main/mcp/health-server/server.mjs) keeps a
mirrored `HEALTH_TYPES` list with a lockstep comment.

## The data API — `/api/health/*`

The health routes follow the board idioms (`force-dynamic`; reads ungated; `storeErrorToResponse`
mapping `NotFoundError → 404`), with the **token gate on writes** (push/delete) standing in for the
usual actor gate:

| Method + route | What it does | Auth |
|---|---|---|
| `POST /api/health/push` | ingest a HAE batch (workouts/metrics/native), dedup by id, purge > 90 days | **`x-health-token`** + add-on gate |
| `GET /api/health/data?type=&from=&to=&limit=` | list raw entries (newest-first; `limit<=0` = all) | ungated |
| `DELETE /api/health/data` | delete entries by `ids` and/or date range | **`x-health-token`** + add-on gate |
| `GET /api/health/summary?date=\|from=&to=` | the aggregated [summary envelope](#the-summarize-contract) | ungated |
| `GET /api/health/daily-summary?date=` | one day of health **folded with nutrition** (see below) | ungated |
| `GET /api/health/trends?days=&type=` | per-day series over the last N days | ungated |
| `GET /api/health/report?days=` | a human-readable **Markdown** report (for vault ingestion) | ungated |

### The `summarize()` contract

`summarize()` in [`board/lib/health.ts`](https://github.com/philipyaz/cos/blob/main/board/lib/health.ts)
...
`/athlete` is where the raw biometrics become coaching. It hangs off one **singleton profile** and a
...
both in [`board/lib/health.ts`](https://github.com/philipyaz/cos/blob/main/board/lib/health.ts), and
served by **`/api/athlete`** (`GET` ungated, `PUT`/`POST` gated).
...
**`GET /api/athlete/form-score?date=`** is a thin wrapper over `computeFormScore` in
[`board/lib/athlete-score.ts`](https://github.com/philipyaz/cos/blob/main/board/lib/athlete-score.ts)
...
with a `level` / `color` / `recommendation`. It is the headline widget on both `/health` and
`/athlete`, and the weekly review + pre-workout brief call it **in-process**.

### The AI coach

Three routes turn the profile + recent health (+ nutrition, when present) into structured coaching.
They share
[`board/lib/athlete-ai.ts`](https://github.com/philipyaz/cos/blob/main/board/lib/athlete-ai.ts)'s
hardened `callClaude`:
...
|---|---|
| `POST /api/athlete/training-plan` | a personalized **weekly plan** (per-day sport/duration/intensity/zones), adapted to the recovery state (HRV/sleep/load) and the profile's constraints (days/equipment/max session) |
| `POST /api/athlete/weekly-review` | a **weekly review** — an `overall_score`, training-vs-plan, sleep trend, recovery/fatigue, and 3–5 recommendations — folding the [form score](#the-daily-form-score) and (soft) nutrition |
| `POST /api/athlete/pre-workout-brief` | a same-day **readiness brief** — ready / cautious / rest-recommended, a recommended session, warnings, and green-lights |

A generated plan can be **materialized onto the calendar** via
**`POST /api/athlete/push-plan-to-calendar`**, which mints `db.events` (gated on the health add-on,
validated day-by-day inside `mutate()`) — the same opt-in calendar bridge nutrition uses for meals.

### Correlations

**`GET /api/athlete/correlations?days=N`**
([source](https://github.com/philipyaz/cos/blob/main/board/app/api/athlete/correlations/route.ts))
...
Health **SOFT-depends** on [Nutrition & Chef](nutrition.md) — declared in the manifest as
...
Crucially, the dependency **does not gate reads**. `/api/health/daily-summary` reads `db.foodLogs`
...
## The health MCP — the agent's read verbs

A new **stdio MCP server** (registry name **`health`**, bridge port **`8011`**,
[`mcp/health-server/server.mjs`](https://github.com/philipyaz/cos/blob/main/mcp/health-server/server.mjs))
is the agent's twin of the dashboard — a **thin `fetch` wrapper** over the `/api/health/*` routes on
`CRM_BASE_URL` (default `http://localhost:3000`), built on `packages/mcp-kit`. It holds **no
business logic** (the report Markdown is composed by `/api/health/report`, not the MCP), makes **no
LLM calls**, and — unlike the nutrition MCP — authenticates its **writes with `x-health-token`**
...
Enabling the add-on reveals a **Health & Athlete** nav group, plus the reachable **Add-ons** catalog
link:

- **`/health`** — the dashboard (today's form score, recent workouts, sleep, HRV, steps, trends).
- **`/athlete`** — the profile + form score + entry points to the coach.
- **`/athlete/training-plan`**, **`/athlete/weekly-review`**, **`/athlete/pre-workout-brief`**,
  **`/athlete/correlations`** — the four coaching surfaces.
...
The add-on's bridge is wired by the **`health-mcp-setup`** skill (the `mcp.setupSkill` in the
manifest) — it renders + loads the launchd bridge plist on `:8011` and wires the `.mcp.json` entry,
exactly like the nutrition bridge. The push token (`HEALTH_PUSH_TOKEN`) lives in
`config/secrets.env`; point Health Auto Export at `/api/health/push` with that token in the
`x-health-token` header.

## Parity rule

Health obeys the board's founding tenet — **one store, one write path** — with one deliberate
...
resolve to the same `/api/health/*` routes, and disabling the add-on freezes writes while leaving
