...
| `board/.claude/skill-bundles/*.zip` — or, with an upload receipt, any zip whose sha256 differs from the one last marked uploaded here | **manual:** upload each in Cowork → Settings → Capabilities → Skills, then `node scripts/mark-skill-uploaded.mjs <skill>…` | Cowork installs the zip you uploaded, not the repo; the board reads the installed cache back and reports drift (Devices page / `get_device_status`, ops#117) — the per-machine receipt (`mcp/logs/.cowork-skills-uploaded.json`) is what this checklist computes the upload list from |
